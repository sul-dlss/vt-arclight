...
RSpec.describe ExploreFacetComponent, type: :component do
  ...
  before do
    render_inline(described_class.new(facet: blacklight_config.facet_fields['media_type'],
                                      image: 'explore-facet-media-format.png',
                                      values: ['Graphic Materials', 'Audio', 'Text',
                                               'Moving Images']))
  end

  # Labels will be modified from those above by the :media_type_label helper
  it "renders the Media type facet labels and link" do
